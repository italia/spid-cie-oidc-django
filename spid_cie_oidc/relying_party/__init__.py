...
from .utils import decode_token, validate_jwt
...
class OAuth2BaseView(object):
    def validate_jwt(self, authz, jwt, keyjar):
        _msg = (
            f"Something went wrong with {authz} JWT validation: "
    ...
    def decode_jwt(self, name, authz, jwt, keyjar):
        try:
            ...
            logger.warning(f"{name} from {authz.issuer} is not in JWT format: {jwt}")
